                    ...
                    unsigned int xor)	    	    /* history xor address flag */
{
  ...
  case BPredPerc:
    if (!l1size)       fatal("the perceptrons number, `%d', must be non-zero and positive",l1size);
	  if (!l2size)       fatal("the perceptrons number, `%d', must be non-zero and positive", l2size);
	  if (!shift_width)  fatal("shift register width, `%d', must be non-zero and positive", shift_width);
	
	pred_dir->config.perc.weight_i    = l1size;
	pred_dir->config.perc.weight_bits = l2size;
	pred_dir->config.perc.history     = shift_width;
	
  // Initializing the weight table with zeros, and the mask table with ones
	int j,k;

    for (j = 0; j < pred_dir->config.perc.i; j++) {
	    for (k=0; k < shift_width; k++)
	    pred_dir->config.perc.weight_table[j][k] = 0;	 	
	  }
        
     for (cnt = 0; cnt < shift_width; cnt++) {
        pred_dir->config.perc.mask_table[cnt] = 1;
     }
    break;
    // -Project ///////////////////////////////////////////// Perceptron //////
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPredPerc:
    fprintf(stream, "pred_dir: %s:  %d Perceptron Prdictor, %d weight_bits, %d history\n",
      name, pred_dir->config.perc.weight_i, pred_dir->config.perc.weight_bits, pred_dir->config.perc.history);
    break;
    // -Project ///////////////////////////////////////////// Perceptron //////
...
// -Project ///////////////////////////////////////////// Comb2Level //////
// BPredComb2Lev: In this section, the configuration and statistics for the 
// combined two-level predictor are set up. It includes configuring the two 
// adaptive predictors and the meta predictor, along with their associated branch 
// target buffers and return address stacks.
  case BPredComb2Lev: 
      bpred_dir_config (pred->dirpred.twolev_1, "TwoLevel A", stream);
      bpred_dir_config (pred->dirpred.twolev_2, "TwoLevel B", stream);
...
// tracking the total number of lookups, updates, and hits. 
  case BPredPerc:
	bpred_dir_config (pred->dirpred.bimod, "perceptron_predictor", stream); 
    fprintf(stream, "btb: %d sets x %d associativity", pred->btb.sets, pred->btb.assoc);
    fprintf(stream, "ret_stack: %d entries", pred->retstack.size);	
	break;
...
// -Project ///////////////////////////////////////////// Comb2Level //////
// This function prints the statistics of the combined two-level branch predictor.
// It reports the address prediction rate and the direction prediction rate, 
    case BPredComb2Lev:
      name = "bpred_comb2lev";
      break;
// -Project ///////////////////////////////////////////// Comb2Level //////
...
// -Project ///////////////////////////////////////////// Comb2Level //////
  if (pred->class == BPredComb2Lev)
    {
      sprintf(buf, "%s.used_twolev_1", name);
      stat_reg_counter(sdb, buf, "total number of bimodal predictions used", &pred->used_twolev_1, 0, NULL);

      sprintf(buf, "%s.used_twolev_2", name);
      stat_reg_counter(sdb, buf, "total number of 2-level predictions used", &pred->used_twolev_2, 0, NULL);
    }
// -Project ///////////////////////////////////////////// Comb2Level //////
...
// sum (w[i]*x[i]) is calculated by multiplying each weight with the corresponding history bit and adding them together
			...
			for (j=0; j < pred_dir->config.perc.history; j++) {// for each bit in the history
				...
				product[j] = (pred_dir->config.perc.weight_table[index][j]) * (pred_dir->config.perc.mask_table[j]); // multiply the weight with the corresponding history bit
...
// -Project ///////////////////////////////////////////// Comb2Level //////
	...
	{
	  char *twolev_1, *twolev_2, *meta;

	  twolev_1 = bpred_dir_lookup (pred->dirpred.twolev_1, baddr);    // get a pointer to direction-prediction bits of predictor 1
	  twolev_2 = bpred_dir_lookup (pred->dirpred.twolev_2, baddr);    // get a pointer to direction-prediction bits of predictor 2

	  meta = bpred_dir_lookup (pred->dirpred.meta, baddr);

	  dir_update_ptr->pmeta = meta; 
	  dir_update_ptr->dir.meta      = (*meta >= 2);         // if meta predictor is >= 2, then use predictor 1, else use predictor 2
	  dir_update_ptr->dir.twolev_1  = (*twolev_1  >= 2);    // if predictor 1 is >= 2, then predict taken, else predict not taken
	  dir_update_ptr->dir.twolev_2  = (*twolev_2 >= 2);     // if predictor 2 is >= 2, then predict taken, else predict not taken

	  if (*meta >= 2)
	    {
	      dir_update_ptr->pdir1 = twolev_1;
	      dir_update_ptr->pdir2 = twolev_2;
	    }
	  else
	    {
	      dir_update_ptr->pdir1 = twolev_2;
	      dir_update_ptr->pdir2 = twolev_1;
	    }
	}
      break;
// -Project ///////////////////////////////////////////// Comb2Level //////
...
// -Project ///////////////////////////////////////////// Comb2Level //////
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->class == BPredComb2Lev))
    {
      int l1index, shift_reg;
      
      /* also update appropriate L1 history register */
      l1index = (baddr >> MD_BR_SHIFT) & (pred->dirpred.twolev_1->config.two.l1size - 1);
      shift_reg = (pred->dirpred.twolev_1->config.two.shiftregs[l1index] << 1) | (!!taken);
      pred->dirpred.twolev_1->config.two.shiftregs[l1index] = shift_reg & ((1 << pred->dirpred.twolev_1->config.two.shift_width) - 1);

      l1index = (baddr >> MD_BR_SHIFT) & (pred->dirpred.twolev_2->config.two.l1size - 1);
      shift_reg = (pred->dirpred.twolev_2->config.two.shiftregs[l1index] << 1) | (!!taken);
      pred->dirpred.twolev_2->config.two.shiftregs[l1index] = shift_reg & ((1 << pred->dirpred.twolev_2->config.two.shift_width) - 1);
    }
// -Project ///////////////////////////////////////////// Comb2Level //////
  ...
  if (pred->class == BPredPerc)
  {
    ...
    // and we need to update the weights
			...
			for (j=0; j < pred->dirpred.bimod->config.perc.history; j++)  // for each bit in the history
      {
				...
				if (pred->dirpred.bimod->config.perc.weight_table[index][j] > 127)  // clamp the weight for this specific implementation to avoid overflow
				pred->dirpred.bimod->config.perc.weight_table[index][j] = 127;      

				if (pred->dirpred.bimod->config.perc.weight_table[index][j] < -128) // clamp the weight for this specific implementation to avoid overflow
				pred->dirpred.bimod->config.perc.weight_table[index][j] = -128;
			}

      // why are we shifting below? 
      //
...
// -Project ///////////////////////////////////////////// Comb2Level //////
      if (pred->class == BPredComb2Lev) {
        if(dir_update_ptr->dir.twolev_1 !=dir_update_ptr->dir.twolev_2) {
          if(dir_update_ptr->dir.twolev_2 == (unsigned int) taken){
