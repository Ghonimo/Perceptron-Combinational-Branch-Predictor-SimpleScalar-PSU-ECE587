...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_string(odb, "-bpred",
		 "branch predictor type {nottaken|taken|bimod|2lev|comb|comb2lev}",
  ...
  // -Project /////////////////////////////////////////////
  opt_reg_int_list(odb, "-bpred:comb2lev",
        "Combined 2-level predictor config "
		   "(<l1size> <l2size> <hist_size> <xor>)",
                   BPredComb2Lev_config, BPredComb2Lev_nelt, &BPredComb2Lev_nelt,
		   /* default */BPredComb2Lev_config,
                   /* print */TRUE, /* format */NULL, /* !accrue */FALSE);
  // -Project /////////////////////////////////////////////
...
// -Project /////////////////////////////////////////////
  else if (!mystricmp(pred_type, "comb2lev"))
    {
      /* combining predictor, bpred_create() checks args */
      if (BPredComb2Lev_nelt != 9)
        fatal("bad combined 2-level pred config (<l1size> <l2size> <hist_size> <xor>)");
      if (comb_nelt != 1)
        fatal("bad combining predictor config (<meta_table_size>)");
      if (btb_nelt != 2)
        fatal("bad btb config (<num_sets> <associativity>)");

      pred = bpred_create_BPredComb2Level(BPredComb2Lev,
              BPredComb2Lev_config[0],
              BPredComb2Lev_config[1],
              BPredComb2Lev_config[4],
              BPredComb2Lev_config[5],
              BPredComb2Lev_config[8],
              BPredComb2Lev_config[2],
              BPredComb2Lev_config[6],
              BPredComb2Lev_config[3],
              BPredComb2Lev_config[7],
              btb_config[0],
              btb_config[1],
              ras_size
              );
